...
1. **URLs are hierarchical.** `/customers/123` means `/customers` is a valid,
   navigable page. Never create a deep route without a parent index page.
2. **Detail / edit / new pages** must render `<Breadcrumbs />` as the **first
   element of content** — above the page heading.
3. **List pages** (first level, accessible from sidebar) do **not** need breadcrumbs.
   Neither do `Dashboard` or `Settings` (they have their own tab navigation).
4. **First crumb is always "Dashboard"** linking to `/dashboard`.
   Last crumb is the current page — **bold, no link**.
   Middle crumbs link to the corresponding list page.
5. **Dynamic segments** (e.g. `/customers/[id]`) must pass `resolveLabel` to
   map the ID to a human-readable name:
   ```tsx
   <Breadcrumbs resolveLabel={(seg) => customer?.fullName ?? seg} />
   ```
6. **Route groups** like `(auth)`, `(dashboard)` are invisible in the URL
   and are automatically stripped from breadcrumbs.
7. **`SEGMENT_LABELS`** in `src/components/layout/breadcrumbs.tsx` must be
   updated when adding a new static route.

### Breadcrumb chains by page type

| Page type | Example path           | Chain                                |
| --------- | ---------------------- | ------------------------------------ |
| List      | `/customers`           | — not needed                         |
| Detail    | `/customers/[id]`      | Dashboard › Customers › Name         |
| Edit      | `/customers/[id]/edit` | Dashboard › Customers › Name › Edit  |
| Create    | `/customers/new`       | Dashboard › Customers › New Customer |

### Pattern: Detail page with breadcrumbs
...
export default async function CustomerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Breadcrumbs resolveLabel={(seg) => customer?.fullName ?? seg} className="mb-4" />
...
- ❌ Breadcrumbs on list pages or Dashboard
- ❌ Detail/edit/new pages without `<Breadcrumbs />`
- ❌ First crumb is "Home" — must be "Dashboard" → `/dashboard`
- ❌ Last crumb has a link — it must be plain bold text (current page)
- ❌ Creating `/customers/[id]/invoices` without a `/customers/[id]/invoices` index page
- ❌ Non-descriptive slugs (`/p/123` instead of `/customers/123`)

---

## Canonical Links — No Broken Routes

**Rule:** Before adding any `href`, confirm the target route exists in the file system.
...
| Create | `/{section}/new`                                           |
| File   | Direct signed URL from storage via `getPublicUrl(fileKey)` |

### Common mistakes to avoid

```tsx
// ❌ WRONG — page /customers/[id] may not exist yet
<Link href={`/customers/${record.id}`}>Open</Link>

// ❌ WRONG — using internal path for a file stored in Supabase Storage
<a href={`/documents/${record.fileKey}`}>Download</a>

// ✅ CORRECT — link to page only if the page.tsx exists
<Link href={`/customers/${customer.id}`}>View Customer</Link>

// ✅ CORRECT — file from storage always uses the public/signed URL
const { data } = supabase.storage.from("invoices").getPublicUrl(record.storagePath)
<a href={data.publicUrl} target="_blank" rel="noreferrer">Download PDF</a>
```

### Rules

1. **Verify the route exists** before linking: `src/app/{section}/[id]/page.tsx` must be present.
2. **Files from storage** (Supabase Storage, S3) — always use `getPublicUrl()` or signed URL.
   Never construct a fake internal path.
3. **External URLs and storage URLs** — always add `target="_blank" rel="noreferrer"`.
4. **Refactoring checklist:**
   - Find all `<a href>` and `<Link href>` in list/card components
   - For each: confirm the target `page.tsx` exists
   - Replace file-path links with `getPublicUrl()` calls
   - Add `<Breadcrumbs />` to all detail/edit/new pages missing it
