...
export const CATS = ["All", "Guides", "Tutorials", "Destinations", "Industry", "Business", "Tech"] as const;

export function avatarFor(name: string): { initials: string; bg: string; fg: string } {
  const initials = name.split(" ").map((p) => p[0]).slice(0, 2).join("").toUpperCase();
  const hue = (name.charCodeAt(0) * 7) % 360;
  return { initials, bg: `hsl(${hue}, 60%, 30%)`, fg: `hsl(${hue}, 80%, 80%)` };
}

export function catEmoji(cat: string): string {
  const map: Record<string, string> = { Guides: "📘", Tutorials: "🛠", Destinations: "✈️", Industry: "📡", Business: "💼", Tech: "⚡" };
  return map[cat] ?? "📝";
}
